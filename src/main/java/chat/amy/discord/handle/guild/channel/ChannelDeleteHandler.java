 ...
 * @since 10/14/17.
 */
public class ChannelDeleteHandler implements CachedEventHandler, JsonCached {
    @Override
    public void handle(final CachedEventContext ctx) {
        final Channel channel = readJsonEvent(ctx.getEvent(), Channel.class);
        ctx.getShard().getCacheController().getMapper(channel).unmap(channel.getId(), Channel.class);
    }
}